...
public class SimBoard
{
	...
	public SimBoard(Country [] originalCountries, BoardState boardState, SimAgent [] simAgents, CardManager cardManager)
	{
	...
	protected void setupCardState(int [] numberOfPlayerCards)
	{
		...
		for(int player = 0; player < numberOfPlayers; player++)
		{
			...
			else
			{
				...
				{
					playersCards.add(cardManager.simDeal());
				}
			}
		}
	}
	...
	public boolean cashCards(Card card1, Card card2, Card card3)
	{
		...
		cardManager.simCash(card1, card2, card3);
		
		//TODO Different versions
		numberOfPlaceableArmies += 5;
	...
	private void playerDefeated(int player, int conquerer)
	{
		...
		{
			cardManager.simReturnToDeck(playerCards.get(player));
		}
